  ...
  const selectCategory = (type: CardType) => {
    ...
    setTimeout(() => {
      ...
      setState(prev => ({ 
        ...
        recentCards: [content.description, ...prev.recentCards].slice(0, 5)
      }));
    }, 400);
  };
  ...
  const getFallback = (type: CardType): CardContent => {
    ...
    if (combinedPool.length === 0) {
      return {
        ...
        description: "No challenges available in this category.",
        difficulty: state.selectedDifficulty
      };
    }

    // Attempt to find a card that wasn't shown in the last 5 rounds
    const availablePool = combinedPool.filter(text => !state.recentCards.includes(text));
    
    // If pool is exhausted by recent filter, fallback to the full pool (rare)
    const finalPool = availablePool.length > 0 ? availablePool : combinedPool;
