...
export const Settings: React.FC<SettingsProps> = ({ state, onUpdate, onClose }) => {
  ...
  return (
    <div 
      className={`fixed inset-0 z-[150] flex flex-col h-full max-w-lg mx-auto p-6 rounded-3xl shadow-2xl overflow-y-auto animate-in fade-in zoom-in-95 duration-200 ${containerClasses}`}
      dir={isRtl ? 'rtl' : 'ltr'}
    >
      ...
      <div className="space-y-6">
        ...
        <section>
          <div className={`flex items-center justify-between p-4 rounded-2xl border ${sectionBg} ${borderClass}`}>
            <div className="flex items-center gap-3">
              <span className="text-xl">🎵</span>
              <h3 className={`font-bold ${textPrimary}`}>{t.bgMusic}</h3>
            </div>
            <button
              onClick={() => onUpdate({ isMusicEnabled: !state.isMusicEnabled })}
              className={`w-14 h-8 rounded-full transition-colors relative ${state.isMusicEnabled ? 'bg-emerald-500' : 'bg-slate-300'}`}
            >
              <div className={`absolute top-1 w-6 h-6 bg-white rounded-full transition-all shadow-md ${state.isMusicEnabled ? (isRtl ? 'left-1' : 'right-1') : (isRtl ? 'right-1' : 'left-1')}`} />
            </button>
          </div>
        </section>
      ...
      <button
        onClick={onClose}
        className={`mt-10 mb-6 font-bold py-4 rounded-2xl shadow-xl hover:scale-[1.02] active:scale-95 transition-all text-lg ${state.isDarkMode ? 'bg-white text-slate-900' : 'bg-slate-900 text-white'}`}
      >
        {t.backToGame}
      </button>
    </div>
