...
import { FALLBACK_CARDS_BY_LANG, CARD_STYLES, CARD_TITLES, UI_TEXT, INTENSITY_INFO, DIFFICULTY_INFO } from './constants';
...
const App: React.FC = () => {
  ...
  const selectCategory = async (type: CardType) => {
    ...
    if (state.useAI) {
      ...
      if (aiResult) {
        const localizedTitle = CARD_TITLES[state.language]?.[type] || aiResult.title || type;
        content = { type, title: localizedTitle, description: aiResult.description, difficulty: state.selectedDifficulty };
      } else {
  ...
  const getFallback = (type: CardType): CardContent => {
    const langCards = FALLBACK_CARDS_BY_LANG[state.language] || FALLBACK_CARDS_BY_LANG['fr'] || FALLBACK_CARDS_BY_LANG['en'];
    const diffPool = langCards[state.selectedDifficulty] || langCards[Difficulty.EASY];
    const intensityPool = diffPool[state.selectedIntensity] || diffPool[Intensity.CASUAL];
    const pool = intensityPool[type] || [];
    const text = pool.length > 0 ? pool[Math.floor(Math.random() * pool.length)] : "Challenge!";
    const title = CARD_TITLES[state.language]?.[type] || type;
    return {
      type,
      title,
      description: text,
  ...
  const t = UI_TEXT[state.language] || UI_TEXT.fr;

  const currentIntensityLabel = typeof INTENSITY_INFO[state.selectedIntensity]?.label === 'object'
    ? (INTENSITY_INFO[state.selectedIntensity].label[state.language] || INTENSITY_INFO[state.selectedIntensity].label.en)
    : INTENSITY_INFO[state.selectedIntensity]?.label;

  const currentDifficultyLabel = typeof DIFFICULTY_INFO[state.selectedDifficulty]?.label === 'object'
    ? (DIFFICULTY_INFO[state.selectedDifficulty].label[state.language] || DIFFICULTY_INFO[state.selectedDifficulty].label.en)
    : DIFFICULTY_INFO[state.selectedDifficulty]?.label;
    ...
    <div className={`h-screen w-full flex flex-col overflow-hidden relative p-4 sm:p-8 transition-colors duration-500 ${themeClasses}`}>
          ...
          <header className="relative flex items-center justify-between mb-10 w-full px-4">
            ...
            <div className="text-center">
              ...
              <p className={`${state.isDarkMode ? 'text-slate-400' : 'text-slate-500'} font-bold uppercase text-[10px] tracking-tighter`}>
                {t.intensity}: <span className="text-emerald-500">{currentIntensityLabel}</span> | {t.mode}: <span className="text-emerald-500">{currentDifficultyLabel}</span> | {t.ai}: <span className={state.useAI ? 'text-emerald-500' : 'text-rose-500'}>{state.useAI ? t.on : t.off}</span>
              </p>
            </div>
            <button 
              ...
              <div className="animate-in zoom-in-95 duration-500 flex flex-col items-center px-4">
                ...
                <div className="grid grid-cols-2 gap-4 w-full">
                  {(Object.keys(CARD_STYLES) as CardType[]).map((type) => {
                    const cardTitle = CARD_TITLES[state.language]?.[type] || type;
                    return (
                      <button
                        key={type}
                      ...
                      >
                        ...
                        <span className="font-black text-white text-lg uppercase">{cardTitle}</span>
      ...
      {state.mode !== GameMode.SETUP && state.mode !== GameMode.SETTINGS && (
        <div className="mt-auto flex justify-center py-4">
          ...
          >
            ← {state.language === 'fr' ? 'Réinitialiser les joueurs' : state.language === 'ar' ? 'إعادة ضبط اللاعبين' : 'Reset Players'}
          </button>
        </div>
      )}
    </div>
